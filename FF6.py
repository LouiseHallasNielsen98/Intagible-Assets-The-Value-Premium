...
for col in cum_cols:
    factor_name = col.replace('_cum_your_model', '')
    color = color_map.get(factor_name, 'black') 
    axes[0].plot(factor_returns['date'], factor_returns[col], label=factor_name, color=color)

axes[0].set_title('Cumulative Returns of All Factors')
axes[0].set_xlabel('Date')
axes[0].set_ylabel('Cumulative Return')
axes[0].legend()
axes[0].grid(True)

# Plot 2: Cumulative Returns: HML vs. HML(INT)
axes[1].plot(factor_returns['date'], factor_returns['HML_cum_your_model'], label='HML', color=color_map['HML'])
axes[1].plot(factor_returns['date'], factor_returns['HML_INT_cum_your_model'], label='HML_INT', color=color_map['HML_INT'])

axes[1].set_title('Cumulative Returns: HML vs. HML(INT)')
